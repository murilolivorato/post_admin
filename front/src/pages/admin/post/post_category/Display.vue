                                        ...
                                        <div class="card-body" >
                                                ...
                                                <table class="table">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <post-category-display v-for="(postcategory , index ) in displayItems"
                                                                              :index="displayItems.indexOf(postcategory)"
                                                                              :list="postcategory"
                                                                              :key="index" ></post-category-display>
...
import PostCategoryDisplay from './inc/PostCategoryDisplay'
...
export default {
  ...
  methods: {
    deleteManyItems () {
      this.$store.dispatch('adminPostCategory/setManySelectItemByIndex', this.selectedItems)
      this.openModel('delete')
    },
    loadFilters (data) {
      // shows
      Promise.all([
        this.$store.dispatch('adminPostCategory/setFilter', { value: data.value, name: data.name }),
        this.$store.dispatch('adminPostCategory/load_pages')
      ]).then((res) => {
        // SE STATUS LOADING PAGE - FALSE
        this.$store.dispatch('adminPostCategory/change_loading_page_status', false)
      })
    },
    clearFilters () {
      ...
      // shows
      Promise.all([
        this.$store.dispatch('adminPostCategory/reset_all_filters'),
        // LOAD THE PAGE
        this.$store.dispatch('adminPostCategory/load_pages', { firstLoad: false }),
        // MAKE FANCY URL
        this.$store.dispatch('adminPostCategory/makeFancyURL', false)
      ]).then((res) => {
        // LOADING PAGE IS FALSE
    ...
    loadPage () {
      ...
      Promise.all([
        ...
        // CHENGE PAGE ONE
        this.$store.dispatch('adminPostCategory/setFirstPaginationPagination')
        ...
        Promise.all([
          ...
          // SET FILTERS
          this.$store.dispatch('adminPostCategory/set_filters'),
    ...
    changePage (page) {
      ...
      Promise.all([
        // SET FILTERS
        this.$store.dispatch('adminPostCategory/changePagePagination', page),
        // LOAD THE PAGE
        this.$store.dispatch('adminPostCategory/load_pages', { firstLoad: false }),
        // MAKE FANCY URL
        this.$store.dispatch('adminPostCategory/makeFancyURL', false)
  ...
  computed: {
    ...
    filterTitle: {
      ...
      set (value) {
        return this.$store.dispatch('adminPostCategory/setFilter', { value: value, name: 'title' })
      }
    }
  ...
  mounted: function () {
    ...
    if (this.$route.params.init) {
      this.$store.dispatch('adminPostCategory/changePagePagination', 1)
    }
