                ...
                <div class="card">
                    ...
                    <div class="card-body" >
                        ...
                        <table class="table">
                            ...
                            <post-user-display v-for="( postuser , index )  in displayItems"
                                                   :index="displayItems.indexOf(postuser)"
                                                   :list="postuser"
                                                   :key="index" ></post-user-display>
...
import PostUserDisplay from './inc/PostUserDisplay'
...
export default {
  ...
  methods: {
    deleteManyItems () {
      this.$store.dispatch('adminPostUser/setManySelectItemByIndex', this.selectedItems)
      this.openModel('delete')
    },
    loadFilters (data) {
      // shows
      Promise.all([
        this.$store.dispatch('adminPostUser/setFilter', { value: data.value, name: data.name }),
        this.$store.dispatch('adminPostUser/load_pages')
      ]).then((res) => {
        // SE STATUS LOADING PAGE - FALSE
        this.$store.dispatch('adminPostUser/change_loading_page_status', false)
      })
    },
    clearFilters () {
      ...
      // shows
      Promise.all([
        this.$store.dispatch('adminPostUser/reset_all_filters'),
        // LOAD THE PAGE
        this.$store.dispatch('adminPostUser/load_pages', { firstLoad: false }),
        // MAKE FANCY URL
        this.$store.dispatch('adminPostUser/makeFancyURL', false)
      ]).then((res) => {
        // LOADING PAGE IS FALSE
    ...
    loadPage () {
      ...
      Promise.all([
        // CLEAR FILTERS
        this.$store.dispatch('adminPostUser/clear_all_filters'),
        // CHENGE PAGE ONE
        ...
        Promise.all([
          ...
          // SET FILTERS
          this.$store.dispatch('adminPostUser/set_filters'),
    ...
    changePage (page) {
      ...
      Promise.all([
        // SET FILTERS
        this.$store.dispatch('adminPostUser/changePagePagination', page),
        // LOAD THE PAGE
        this.$store.dispatch('adminPostUser/load_pages', { firstLoad: false }),
        // MAKE FANCY URL
        this.$store.dispatch('adminPostUser/makeFancyURL', false)
  ...
  computed: {
    ...
    filterTitle: {
      ...
      set (value) {
        return this.$store.dispatch('adminPostUser/setFilter', { value: value, name: 'title' })
      }
    }
  ...
  mounted: function () {
    ...
    if (this.$route.params.init) {
      this.$store.dispatch('adminPostUser/changePagePagination', 1)
    }
