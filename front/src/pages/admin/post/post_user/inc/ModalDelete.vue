    ...
    <modal  @close="closeDeleteModal()"  :closeoutside="true" >
        ...
        <template slot="body" >
            ...
            <div v-if="componentIsLoaded">
                ...
                <!-- ELSE -->
                <div v-else>
                    <p v-if="selectionList.length"> Existem uma ou mais Notícias Cadastradas neste usuário , você precisa primeiro deletar as Notícias , para depois deletar o usuário .   ?</p>
                    <p v-if="! selectionList.length">Existe uma ou mais Notícias Cadastradas neste usuário , você precisa primeiro deletar a Notícia , para depois deletar o usuário .</p>
                    <div class="modal-footer">
                        <button type="button" class="lightOne"  @click="closeDeleteModal()" >Fechar</button>
...
export default {
  ...
  methods: {
    ...
    storeForm () {
      ...
      this.$store.dispatch('adminPostUser/destroy', { delete: this.selectionList }).then((res) => {
