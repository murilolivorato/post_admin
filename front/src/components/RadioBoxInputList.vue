<template>
    <div class="block-item cboxtags-list">
        ...
        <ul class="ks-cboxtags" v-if="componentIsLoaded">
                <li v-if="emptytext">
                    <input  type="radio"  :value="emptyValue" :id="namelist"   v-model="listValue" >
                    <label :for="namelist">  <font-awesome-icon :icon="verifyContain(emptyValue)"  /> {{ emptytext }}</label>
                </li>

                <!-- li -->
                <li   v-for="( feature , index ) in list" v-if="showList(index)" >
                    <input  type="radio"  :value="feature.id" :id="namelist + feature.id"   v-model="listValue" >
...
<script>
export default {
  name: 'RadioBoxInputList',
  props: {
    list: { type: Array },
    checkedvalue: { type: [String, Number], default: null },
